-- U6: a quantidade de order_items continua sendo sempre física/canônica.
-- A apresentação comercial é um snapshot opcional; linhas históricas e Shopee
-- continuam válidas como entrada direta (factor 1 implícito).

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS commercial_presentation_id uuid NULL REFERENCES public.commercial_presentations(id),
  ADD COLUMN IF NOT EXISTS commercial_presentation_name text NULL,
  ADD COLUMN IF NOT EXISTS commercial_unit_label text NULL,
  ADD COLUMN IF NOT EXISTS commercial_conversion_factor numeric NULL,
  ADD COLUMN IF NOT EXISTS commercial_quantity numeric NULL,
  ADD COLUMN IF NOT EXISTS physical_unit_label text NULL;

ALTER TABLE public.order_items
  DROP CONSTRAINT IF EXISTS order_items_commercial_snapshot_check;

ALTER TABLE public.order_items
  ADD CONSTRAINT order_items_commercial_snapshot_check CHECK (
    (
      commercial_presentation_id IS NULL
      AND commercial_presentation_name IS NULL
      AND commercial_unit_label IS NULL
      AND commercial_conversion_factor IS NULL
      AND commercial_quantity IS NULL
      AND physical_unit_label IS NULL
    ) OR (
      commercial_presentation_name IS NOT NULL
      AND commercial_unit_label IS NOT NULL
      AND physical_unit_label IS NOT NULL
      AND commercial_conversion_factor > 0
      AND commercial_quantity > 0
      AND quantity = commercial_quantity * commercial_conversion_factor
    )
  );

CREATE OR REPLACE FUNCTION public.validate_order_item_commercial_snapshot()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
DECLARE
  v_presentation public.commercial_presentations%ROWTYPE;
BEGIN
  IF NEW.commercial_presentation_id IS NULL THEN
    RETURN NEW;
  END IF;

  SELECT * INTO v_presentation
  FROM public.commercial_presentations
  WHERE id = NEW.commercial_presentation_id;

  IF NOT FOUND
    OR v_presentation.product_id IS DISTINCT FROM NEW.product_id
    OR v_presentation.variant_id IS DISTINCT FROM NEW.variant_id THEN
    RAISE EXCEPTION 'A apresentação comercial não pertence à identidade física do item.';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS order_items_validate_commercial_snapshot ON public.order_items;
CREATE TRIGGER order_items_validate_commercial_snapshot
  BEFORE INSERT OR UPDATE OF product_id, variant_id, commercial_presentation_id
  ON public.order_items
  FOR EACH ROW EXECUTE FUNCTION public.validate_order_item_commercial_snapshot();

-- Mantém o motor único. O preço unitário passa a significar preço por unidade
-- comercial quando há snapshot, e preço por unidade física em linhas legadas.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    IF NULLIF(v_item->>'product_id', '') IS NULL OR COALESCE((v_item->>'quantity')::numeric, 0) <= 0 THEN RAISE EXCEPTION 'Produto ou quantidade física inválida.'; END IF;
    ...
    ) THEN RAISE EXCEPTION 'Snapshot comercial do item inválido.'; END IF;
    INSERT INTO public.order_items(order_id, product_id, variant_id, quantity, unit_price, notes, commercial_presentation_id, commercial_presentation_name, commercial_unit_label, commercial_conversion_factor, commercial_quantity, physical_unit_label)
    VALUES (v_order.id, (v_item->>'product_id')::uuid, NULLIF(v_item->>'variant_id', '')::uuid, (v_item->>'quantity')::numeric, COALESCE((v_item->>'unit_price')::numeric, 0), NULLIF(v_item->>'notes', ''), NULLIF(v_item->>'commercial_presentation_id', '')::uuid, NULLIF(v_item->>'commercial_presentation_name', ''), NULLIF(v_item->>'commercial_unit_label', ''), NULLIF(v_item->>'commercial_conversion_factor', '')::numeric, NULLIF(v_item->>'commercial_quantity', '')::numeric, NULLIF(v_item->>'physical_unit_label', ''));
  END LOOP;
