...
export function evaluateTemporalAuthority(
  ...
  if (schedule.kind === 'crm_reactivation') {
    return { schedule: schedule.kind, decision: 'preserve_reactivation', reason: 'Reativação comercial é obrigação independente da programação operacional da conversa.' };
  }
