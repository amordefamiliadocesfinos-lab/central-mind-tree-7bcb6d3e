...
export function analyzeProductCatalog(rows: ProductCatalogRow[], products: CatalogProduct[], variants: CatalogVariant[], categories: string[]): CatalogAnalysis {
  ...
  for (const row of rows) {
    ...
    if (kind === 'PRODUTO') {
      ...
      const keepsOwnSku = Boolean(text(row.variante_id) && existing && key(existing.sku) === key(sku));
      if (!master && !fileProductSkus.has(key(masterSku))) rowErrors.push('Produto mestre não encontrado'); if (text(row.variante_id) && !existing) rowErrors.push('variante_id não encontrado'); const collisionProduct = productBySku.get(key(sku)); if (collisionProduct && !keepsOwnSku) rowErrors.push('SKU já pertence a um produto mestre'); if (!text(row.variante_id) && variantBySku.get(key(sku))) rowErrors.push('SKU já pertence a uma variação'); if (existing && variantBySku.get(key(sku)) && variantBySku.get(key(sku))?.id !== existing.id) rowErrors.push('SKU já pertence a outra variação');
