...
export function BOMImportDialog({ onImported }: { onImported?: () => void }) {
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      const [rows, productsResult, variantsResult, componentsResult] = await Promise.all([
        ...
        (supabase.from('products') as any).select('id, sku, name, variation_mode, is_active'),
        supabase.from('product_variants').select('id, product_id, sku, variant_name, is_active'),
  ...
  return <><Button type="button" size="sm" variant="outline" onClick={() => setOpen(true)}><Upload className="mr-2 h-4 w-4" />Importar BOM</Button>
    <Dialog open={open} onOpenChange={close}><DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto"><DialogHeader><DialogTitle>Importar composição (BOM)</DialogTitle><DialogDescription>Colunas obrigatórias: produto_sku, produto_variante_sku, componente_sku, componente_variante_sku, quantidade e observacao. A prévia é obrigatória e não movimenta estoque.</DialogDescription></DialogHeader>
