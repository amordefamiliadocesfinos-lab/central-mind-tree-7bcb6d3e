...
import { OrderStockShortageError } from '@/lib/orderStock';
...
export function OrderSeparationBoard({ orders, separationByOrderId, documentsByOrderId, onPrint, onFinalize, onSetDestination, onAttachDocument }: Props) {
  ...
  const [stockShortages, setStockShortages] = useState<OrderStockShortageError | null>(null);
  ...
  return <div className="space-y-4">
        ...
        return <section key={column.status} className={cn('rounded-xl border p-3 min-h-48', column.color)}>
          ...
          <div className="space-y-3">{columnOrders.map(order => {
            ...
            return <Card key={order.id} className="bg-background"><CardContent className="space-y-3 p-3">
              ...
              {order.notes?.trim() && <div className="rounded-md border bg-muted/30 p-2 text-xs"><p className="mb-1 font-medium">Observação do pedido</p><p className="whitespace-pre-wrap text-muted-foreground">{order.notes}</p></div>}
    ...
    <Dialog open={Boolean(stockShortages)} onOpenChange={open => !open && setStockShortages(null)}><DialogContent><DialogHeader><DialogTitle>Estoque insuficiente para finalizar</DialogTitle></DialogHeader><div className="space-y-2 text-sm"><p className="text-muted-foreground">Nenhuma saída física foi registrada. Ajuste o estoque antes de tentar novamente.</p>{stockShortages?.shortages.map(shortage => <div key={`${shortage.product_id}:${shortage.variant_id ?? 'simple'}`} className="rounded-md border p-3"><p className="font-medium">{shortage.product_name ?? 'Produto'}{shortage.variant_name ? ` · ${shortage.variant_name}` : ''}</p><p className="text-muted-foreground">Necessário: {shortage.required_quantity} · Disponível: {shortage.available_quantity} · Faltam: {shortage.missing_quantity}</p></div>)}</div></DialogContent></Dialog>
  </div>;
}
