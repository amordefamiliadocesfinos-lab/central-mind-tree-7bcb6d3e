...
export function usePurchases() {
  ...
  const createPresentation = useCallback(async (input: any) => { const { data, error } = await db.from('purchase_presentations').insert(input).select().single(); if (error) throw error; return data; }, []);
